...
const {findMembers} = require("./common-util");
const isMobilePhone = function (params) {
    return validator.default.isMobilePhone(params, ["zh-CN"])
}
const isFloat = function (params, min = Number.MIN_VALUE, max = Number.MAX_VALUE) {
    return validator.default.isFloat("" + params, {min, max});
}
const isInt = function (params, min = Number.MIN_VALUE, max = Number.MAX_VALUE) {
    return validator.default.isInt("" + params, {min, max});
}
const isLength = function (params, min = 0, max = Number.MAX_VALUE) {
    return validator.default.isLength("" + params, {min, max});
}
...
class LinValidator {
    ...
    async validate(ctx, alias = {}) {
        ...
        for (let key of memberKeys) {
            ...
            if (!result.success) {
                ...
                // throw new ParameterException(result.msg,result.error_code);
            }
        }
    ...
    async _check(key, alias = {}) {
        ...
        if (isCustomFunc) {
            try {
                ...
                result = new RuleResult(
                    ...
                    error.error_code || error.errorCode ||"10001"
                );
            }
            // 函数验证
        } else {
        ...
        if (!result.pass) {
            ...
            const error_code = result.error_code || 10001
            return {
                msg: msg,
                success: false,
...
class RuleResult {
    constructor(pass, msg = "",error_code = "10001") {
        Object.assign(this, {
            pass,
            msg,
            error_code
